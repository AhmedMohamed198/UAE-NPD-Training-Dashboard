...
async function getFlashReports(token, pageId) {
  ...
  if (/^\d{4}$/.test(firstText)) {
    ...
    for (const wb of topC) {
      const title = bt(wb);
      if (title && !seenIds[wb.id]) { seenIds[wb.id] = true; weeks.push({ id: wb.id, title }); }
    }
  }
...
async function parseFlashBlocks(token, blocks) {
  ...
  const detectSection = lower => {
    if (/snapshot|overview/.test(lower) && !/high.?rated|low.?rated|new.?meal|top.?perform|underperform/.test(lower)) return 'snapshot';
  ...
  try {
    switch (action) {
      ...
      case 'getFlashReports': {
        const key = 'NOTION_FLASH_PAGE_' + mp;
        const pageId = process.env[key] || process.env['NOTION_FLASH_PAGE_UAE'];
        if (!pageId) return res.status(200).json({ configured: false, weeks: [], hint: key + ' env var not set' });
      ...
      case 'getMonthlyReviewMonths': {
        const key = 'NOTION_MONTHLY_PAGE_' + mp;
        const pageId = process.env[key] || process.env['NOTION_MONTHLY_PAGE_UAE'];
        if (!pageId) return res.status(200).json({ configured: false, months: [], hint: key + ' env var not set' });
      ...
      case 'debug': {
        const key = 'NOTION_FLASH_PAGE_' + mp;
        const pageId = process.env[key] || process.env['NOTION_FLASH_PAGE_UAE'];
        if (!pageId) return res.status(200).json({ ok: false, error: key + ' env var not set' });
        const pageMeta = await nFetch(TOKEN, '/pages/' + pageId);
        const top = await nChildren(TOKEN, pageId);
        const blocks = top.slice(0, 20).map(b => ({
          id: b.id, type: b.type, text: nBlockText(b), has_children: b.has_children
        }));
        return res.status(200).json({ ok: true, pageId, blocks });
      }
