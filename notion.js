...
module.exports = async function handler(req, res) {
  ...
  try {
    switch (action) {
      ...
      case 'debug': {
        const mpKey = mp && mp !== 'undefined' ? mp : 'UAE';
        const key = 'NOTION_FLASH_PAGE_' + mpKey;
        const pageId = process.env[key];
        if (!pageId) return res.status(200).json({ ok: false, error: key + ' env var not set', mp: mpKey });
        const top = await nChildren(TOKEN, pageId);
        const blocks = top.slice(0, 20).map(b => ({
          id: b.id, type: b.type, text: nBlockText(b), has_children: b.has_children
        }));
        return res.status(200).json({ ok: true, pageId, mp: mpKey, blocks });
      }
