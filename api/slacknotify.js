...
module.exports = async function handler(req, res) {
  ...
  // No-auth health check — open /api/slack-notify?action=ping in a browser to confirm
  // the function is deployed and which env vars are present (no secrets exposed).
  const pingAction = (req.method === 'GET' ? (req.query || {}).action : (req.body || {}).action) || '';
  if (pingAction === 'ping') {
