...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'login') {
      ...
      const users = await ghRead('data/users.json');
      if (!users) return res.status(500).json({ ok: false, error: 'User database not found. Contact admin.' });
    ...
    if (action === 'changePassword') {
      ...
      await (async () => {
        ...
        const r = await fetch(`https://api.github.com/repos/${GH_REPO}/contents/data/users.json`, {
          method: 'PUT',
