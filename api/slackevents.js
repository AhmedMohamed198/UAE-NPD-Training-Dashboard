...
async function handler(req, res) {
  ...
  // Debug: log raw text and blocks so we can see the actual Slack format
  console.log('[slack-events] RAW TEXT:', JSON.stringify(text));
  console.log('[slack-events] BLOCKS:', JSON.stringify(event.blocks || []));
  console.log('[slack-events] ATTACHMENTS:', JSON.stringify(event.attachments || []));

  // ── CASE 1: "Done" reply — mark existing fix as Done ─────────────────────────
  const isDone = /is now fixed on the dashboard/i.test(text) ||
                 /clicked\s+recipe updated/i.test(text);
  ...
  if (isDone) {
    ...
    try {
      ...
      console.log('[slack-events] Done reply — no matching fix found for:', doneName);
    } catch(e) {
  ...
  if (isMealFixesFormat) {
    ...
    fixName = field('Name', text) || field('Meal Name', text) || field('Component', text);
    if (!fixName) {
    ...
    issueDetails = field('Details', text) || field('Description', text) || field('Issue', text);
    notesLink    = field('Link', text) || field('URL', text);
    ...
    // Positional format (Recipe Fixes and Scaling)
    const lines = text.split('\n')
    ...
    // Market line: contains country name
    ...
    if (mpRaw.includes('jeddah'))          mp = 'KSA-Jeddah';
      ...
      console.log('[slack-events] Market not in monitored list:', lines[marketIdx]);
      return res.status(200).end();
    }
    ...
    if (assigneeIdx >= 0) {
      ...
      reportedBy   = middleLines[assigneeIdx].replace(/^@/, '').trim();
    } else {
      issueDetails = middleLines.join(' ');
      reportedBy   = strip(event.username || '');
    }
    notesLink = msgLink;

  } else {
    // Regular user message — route by poster identity
