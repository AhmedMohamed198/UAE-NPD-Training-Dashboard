...
async function handler(req, res) {
  ...
  if (isWorkflow) {
    ...
    // Try "Name X" label first; fall back to the line right before "Apply changes to"
    ...
    if (!fixName) {
      ...
      for (let i = applyIdx - 1; i >= 0; i--) {
        const l = lines[i];
        if (l && !/hello|requested by|there'?s|dashboard|^\s*$/.test(l) && !/<[^>]+>/.test(l)) {
          fixName = l; break;
        }
      }
    }
    ...
    if (!fixName) {
      console.log('[slack-events] Could not extract Name from workflow message. Text:', text.slice(0, 300));
      // Last resort: use first meaningful line as name
      const lines = text.split('\n').map(l => strip(l)).filter(Boolean);
      fixName = lines.find(l => !/hello|requested by|there'?s|dashboard|apply changes/i.test(l)) || 'Slack Fix';
    }
  } else {
