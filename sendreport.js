...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Quick connectivity test — return immediately without calling Slack
    if (d._test) return res.status(200).json({ ok: true, test: true, webhook: !!WEBHOOK });

    const blocks = buildBlocks(d);

    const ctrl = new AbortController();
    const tid  = setTimeout(() => ctrl.abort(), 7000); // 7s max for Slack
    let r;
    try {
